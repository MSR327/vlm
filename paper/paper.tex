...
\begin{tikzpicture}[
    font=\small,
    tensor/.style={draw=black!70, fill=gray!25, line width=0.7pt, rounded corners=1pt},
    convblock/.style={draw=black!80, fill=gray!12, line width=0.8pt, rounded corners=2pt, font=\scriptsize\bfseries, align=center},
    denseblock/.style={draw=black!80, fill=gray!18, line width=0.8pt, rounded corners=2pt, font=\scriptsize\bfseries, align=center},
    latentblock/.style={draw=black!85, fill=gray!28, line width=0.8pt, rounded corners=2pt, font=\scriptsize\bfseries, align=center},
    inputblock/.style={draw=black!80, fill=gray!8, line width=0.8pt, rounded corners=2pt, align=center, inner sep=3pt},
    policyblock/.style={draw=black!85, fill=gray!10, line width=0.8pt, rounded corners=2pt, font=\scriptsize\bfseries, align=center},
    ...
    \draw[draw=black!60, fill=white, rounded corners=5pt, line width=0.9pt] (-2.3, -2.55) rectangle (14.6, 3.2);

    % Stream Background Panels
    \draw[draw=black!25, fill=gray!4, rounded corners=3pt] (-0.1, 0.95) rectangle (9.0, 2.55);
    \node[anchor=south west, font=\scriptsize\bfseries, text=black!75] at (-0.1, 2.58) {Camera Stream (Conv-VAE)};

    \draw[draw=black!25, fill=gray!4, rounded corners=3pt] (-0.1, -1.75) rectangle (9.0, -0.15);
    \node[anchor=north west, font=\scriptsize\bfseries, text=black!75] at (-0.1, -1.78) {LiDAR BEV Stream (Conv-VAE)};

    % ==================== TOP STREAM: CAMERA CONV-VAE ====================
    % Input Camera Node (Native Vector)
    \node[inputblock, minimum width=1.85cm, minimum height=1.05cm] (cam_img) at (-1.2, 1.75) {
        \textbf{Front Camera}\\[1pt]
        \scriptsize RGB / Semantic\\[1pt]
        \ttfamily\tiny $160 \times 80 \times 3$
    };

    % Conv1 (80 x 40 x 32)
    \draw[convblock] (0.5, 1.25) rectangle (1.3, 2.25) node[midway] {Conv1\\$4\times 4$};
    \draw[tensor] (1.4, 1.35) rectangle (1.7, 2.15);
    \node[dimlabel] at (1.55, 2.33) {$80\times 40$};

    % Conv2 (40 x 20 x 64)
    \draw[convblock] (2.1, 1.25) rectangle (2.9, 2.25) node[midway] {Conv2\\$3\times 3$};
    \draw[tensor] (3.0, 1.45) rectangle (3.3, 2.05);
    \node[dimlabel] at (3.15, 2.33) {$40\times 20$};

    % Conv3 (20 x 10 x 128)
    \draw[convblock] (3.7, 1.25) rectangle (4.5, 2.25) node[midway] {Conv3\\$4\times 4$};
    \draw[tensor] (4.6, 1.55) rectangle (4.9, 1.95);
    \node[dimlabel] at (4.75, 2.33) {$20\times 10$};

    % Conv4 (10 x 5 x 256)
    \draw[convblock] (5.3, 1.25) rectangle (6.1, 2.25) node[midway] {Conv4\\$3\times 3$};
    \draw[tensor] (6.2, 1.6) rectangle (6.5, 1.9);
    \node[dimlabel] at (6.35, 2.33) {$10\times 5$};

    % Dense Flatten & Bottleneck
    \draw[denseblock] (6.9, 1.25) rectangle (7.6, 2.25) node[midway] {Dense\\$1024$};
    \draw[latentblock] (8.0, 1.25) rectangle (8.8, 2.25) node[midway] {VAE $\mathbf{z}_{\text{cam}}$\\$\mathbb{R}^{95}$};

    % Connectors Top
    \draw[arr] (cam_img.east) -- (0.5, 1.75);
    \draw[arr] (1.7, 1.75) -- (2.1, 1.75);
    \draw[arr] (3.3, 1.75) -- (3.7, 1.75);
    \draw[arr] (4.9, 1.75) -- (5.3, 1.75);
    \draw[arr] (6.5, 1.75) -- (6.9, 1.75);
    \draw[arr] (7.6, 1.75) -- (8.0, 1.75);

    % ==================== BOTTOM STREAM: 2D BEV LIDAR CONV-VAE ====================
    % Input BEV Node (Native Vector)
    \node[inputblock, minimum width=1.85cm, minimum height=1.05cm] (bev_img) at (-1.2, -0.95) {
        \textbf{LiDAR BEV}\\[1pt]
        \scriptsize 2D Metric Grid\\[1pt]
        \ttfamily\tiny $80 \times 160 \times 1$
    };

    % Conv1 (80 x 40 x 32)
    \draw[convblock] (0.5, -1.45) rectangle (1.3, -0.45) node[midway] {Conv1\\$4\times 4$};
    \draw[tensor] (1.4, -1.35) rectangle (1.7, -0.55);
    \node[dimlabel] at (1.55, -1.58) {$80\times 40$};

    % Conv2 (40 x 20 x 64)
    \draw[convblock] (2.1, -1.45) rectangle (2.9, -0.45) node[midway] {Conv2\\$3\times 3$};
    \draw[tensor] (3.0, -1.25) rectangle (3.3, -0.65);
    \node[dimlabel] at (3.15, -1.58) {$40\times 20$};

    % Conv3 (20 x 10 x 128)
    \draw[convblock] (3.7, -1.45) rectangle (4.5, -0.45) node[midway] {Conv3\\$4\times 4$};
    \draw[tensor] (4.6, -1.15) rectangle (4.9, -0.75);
    \node[dimlabel] at (4.75, -1.58) {$20\times 10$};

    % Conv4 (10 x 5 x 256)
    \draw[convblock] (5.3, -1.45) rectangle (6.1, -0.45) node[midway] {Conv4\\$3\times 3$};
    \draw[tensor] (6.2, -1.1) rectangle (6.5, -0.8);
    \node[dimlabel] at (6.35, -1.58) {$10\times 5$};

    % Dense Flatten & Bottleneck
    \draw[denseblock] (6.9, -1.45) rectangle (7.6, -0.45) node[midway] {Dense\\$1024$};
    \draw[latentblock] (8.0, -1.45) rectangle (8.8, -0.45) node[midway] {VAE $\mathbf{z}_{\text{lidar}}$\\$\mathbb{R}^{95}$};

    % Connectors Bottom
    \draw[arr] (bev_img.east) -- (0.5, -0.95);
    \draw[arr] (1.7, -0.95) -- (2.1, -0.95);
    \draw[arr] (3.3, -0.95) -- (3.7, -0.95);
    \draw[arr] (4.9, -0.95) -- (5.3, -0.95);
    \draw[arr] (6.5, -0.95) -- (6.9, -0.95);
    \draw[arr] (7.6, -0.95) -- (8.0, -0.95);

    % ==================== MIDDLE: NAVIGATION & CONCATENATION ====================
    % Telemetry input
    \node[draw=black!75, fill=gray!8, rounded corners=2pt, font=\scriptsize, align=center] (telem) at (6.6, 0.4) {
        \textbf{Kinematics} $\mathbf{x}_{\text{nav}} \in \mathbb{R}^5$\\
        \tiny $[v_t, d_{\text{center}}, \theta_{\text{heading}}, \delta_{t-1}, a_{t-1}]$
    };

    % Concatenation Block
    \node[draw=black!85, fill=gray!22, line width=0.8pt, rounded corners=3pt, font=\scriptsize\bfseries, align=center, inner sep=4pt] (concat) at (9.6, 0.4) {
        Feature Fusion\\
        \tiny Concatenation\\
        \scriptsize $\mathbf{s}_t \in \mathbb{R}^{195}$
    };

    \draw[arr] (8.8, 1.75) -| (concat.north);
    \draw[arr] (8.8, -0.95) -| (concat.south);
    \draw[arr] (telem.east) -- (concat.west);

    % ==================== POLICY HEAD: PPO ACTOR-CRITIC ====================
    \node[policyblock, minimum width=2.8cm, minimum height=1.25cm, inner sep=3pt] (actor) at (12.4, 1.35) {
        \textbf{PPO Actor Head}\\[1pt]
        \tiny MLP: $500 \text{--} 300 \text{--} 100 \text{--} 2$\\[1pt]
        \scriptsize Actions $\mathbf{a}_t = [\delta_t, \text{acc}]$
    };

    \node[policyblock, fill=gray!18, minimum width=2.8cm, minimum height=1.05cm, inner sep=3pt] (critic) at (12.4, -0.45) {
        \textbf{PPO Critic Head}\\[1pt]
        \tiny MLP: $500 \text{--} 300 \text{--} 100 \text{--} 1$\\[1pt]
        \scriptsize Value Baseline $V_\phi(\mathbf{s}_t)$
    };

    \draw[arr] (concat.east) -- ++(0.3, 0) |- (actor.west);
    \draw[arr] (concat.east) -- ++(0.3, 0) |- (critic.west);

    % CARLA Output and Feedback
    \node[draw=black!80, fill=white, rounded corners=3pt, font=\scriptsize\bfseries, align=center, inner sep=3.5pt, minimum width=2.8cm] (carla) at (12.4, -1.8) {
        \textbf{CARLA Environment}\\[1pt]
        \tiny Closed-loop physics ($20\text{ Hz}$)\\[1pt]
        \tiny $r_t = R_{\text{speed}} \cdot R_{\text{center}} \cdot R_{\text{angle}} + R_{\text{pen}}$
    };
    \draw[arr] (actor.east) -- ++(0.3, 0) |- node[pos=0.75, right, font=\tiny\bfseries] {$\mathbf{a}_t$} (carla.east);
    \draw[arr, dashed, black!60] (carla.west) -- ++(-0.6, 0) |- (-1.2, -2.3) -- (bev_img.south);

\end{tikzpicture}
}
\caption{End-to-end multimodal deep reinforcement learning architecture in CARLA, inspired by TransFuser~\cite{transfuser2021}. Perspective RGB streams and 2D metric Bird's-Eye-View (BEV) LiDAR grids are compressed by frozen Conv-VAE encoders into compact latent representations ($\mathbf{z}_{\text{cam}}, \mathbf{z}_{\text{lidar}} \in \mathbb{R}^{95}$), fused via concatenation with 5-dimensional kinematic vehicle telemetry ($\mathbf{s}_t \in \mathbb{R}^{195}$), and fed into dual PPO actor-critic network heads for continuous $20\text{ Hz}$ closed-loop control.}
...
\textbf{Inference Compute (GMACs) $\downarrow$} & \textbf{0.074} & 0.222 & 0.296 & 0.148 \\
...
Fig.~\ref{fig:route_comp}, \ref{fig:reward_curve}, and~\ref{fig:infraction_chart} illustrate closed-loop training performance across the four sensor ladder rungs in Town01. Rung 1 trained for $\sim$105k steps, achieving $39.0\% \pm 2.4\%$ route completion ($R=313.21 \pm 38.45$). Rung 2 reached $25.0\% \pm 4.1\%$ route completion ($R=158.31 \pm 26.80$) in 12.2k steps---lower completion than the monocular baseline, though the shorter training budget complicates direct comparison. Rung 3 flatlined at $3.4\% \pm 0.8\%$ completion ($R=17.35 \pm 3.12$) in a standstill trap. Rung 4 achieved $-5.02 \pm 4.18$ reward across 230 episodes (13.2k steps) with zero stall terminations but 6.0\% route completion. \textbf{Important caveat:} Rungs 2--4 trained for $8\text{--}10\times$ fewer steps than Rung 1, so performance differences may reflect training duration rather than architectural limits. Fig.~\ref{fig:vae_loss} confirms convergence of the BEV autoencoder to $0.0046$ MSE.
...
\textbf{Deep (Hierarchical)} & 6 & $32 \to 64 \to 64 \to 128 \to 128 \to 256$ & 95 & 29,198,046 & 7.06 & 7.4\% & 0.6913 & 0.8965 & 0.7285 & 248.1 FPS (4.0 ms) \\
\textbf{Deep-190 (Large Latent)} & 6 & $32 \to 64 \to 64 \to 128 \to 128 \to 256$ & 190 & 29,392,796 & 7.14 & 3.8\% & 0.7089 & 0.8847 & 0.7800 & \textbf{258.0 FPS (3.9 ms)} \\
\bottomrule
\end{tabular}
}
\end{table*}
...
\subsubsection{Zero-Shot Policy Misalignment and Closed-Loop Rectification}
In closed-loop CARLA rollouts, hot-swapping newly trained Wide and Deep encoders into the pre-trained PPO policy induced an immediate $100\%$ standstill stall rate ($0.0\text{ m}$ route completion). Investigation identified two root causes: (1)~legacy baseline training omitted KL loss, allowing latent scale inflation ($\sigma_z \approx 17.8$), whereas regularized Wide/Deep models maintain $\sigma_z \approx 0.70$. Feeding $25\times$ under-excited latents into the frozen actor caused outputs to collapse to negative longitudinal bias ($a_{\text{long}} \approx -0.17$), which the evaluation harness mapped directly to continuous braking ($t=0$ deadlock); (2)~Deep-190 was dimensionally incompatible with the 100-dim actor. Standardizing forward throttle mapping, adding anti-standstill logic, and fitting a linear Ridge alignment adapter ($\mathbf{z}_{\text{adapted}} = \mathbf{z}_{\text{target}} \mathbf{W}_{\text{align}} + \mathbf{b}_{\text{align}}$) restores effective policy excitation.
...
\begin{tikzpicture}[
    ...
    % 4. Rung 2: Green trajectory collides with curb at 125m on straightaway
    \draw[green!60!black, line width=1.8pt, -{Stealth[length=2mm]}] (0.0, -0.05) -- (2.0, -0.05) -- (2.5, -0.55);
    \node[text=red, font=\bfseries] at (2.5, -0.55) {\Large $\times$};
    \draw[-Stealth, green!60!black, thin] (2.5, -0.55) -- (2.5, -1.1);
    \node[badge, fill=green!10, draw=green!70, text=green!70!black, anchor=north] at (2.5, -1.15) {\textbf{Rung 2: Curb Impact ($125\text{ m}$)}\\Fails on straightaway};
