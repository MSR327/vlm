    ...
    def step(self, action_idx):
        try:
            ...
            if self.continous_action_space:
                ...
                print(f"Action values : {self.previous_steer*0.9 + steer*0.1},{self.throttle*0.9 + throttle*0.1}")
...
class CameraSensorEnv:
    ...
    def __init__(self, vehicle):
        ...
        pygame.init()
        self.display = pygame.display.set_mode((600, 600),pygame.HWSURFACE | pygame.DOUBLEBUF)
    ...
    def _set_camera_sensor(self, world):
        ...
        thrid_person_camera_bp.set_attribute('image_size_x', f'600')
        thrid_person_camera_bp.set_attribute('image_size_y', f'600')
...
class PPOAgent(tf.keras.Model):
    ...
    def __call__(self, obs, train=False, **kwargs):
        """Keras 3 compatibility shim -- behaviour is unchanged.

        Keras 3 (TF >= 2.16) rejects non-tensor POSITIONAL arguments to
        Layer.__call__:

            ValueError: Only input tensors may be passed as positional
            arguments. The following argument value should be passed as a
            keyword argument: False (of type <class 'bool'>)

        Every existing call site passes the flag positionally --
        main.py `agent(observation, True)` in train(), `agent(observation,
        False)` in test(), the same in capture_data(), and the same pattern
        in test.py / mian_test.py. Rather than edit all of them, this shim
        accepts the positional flag and forwards it as a keyword.

        Nothing else changes: the same `obs` reaches the same `call()`, with
        the same `train` value, and `call()` itself is untouched apart from
        gaining a default for `train`. On Keras 2 this is a no-op passthrough.
        """
        return super().__call__(obs, train=train, **kwargs)

    def call(self, obs, train=False):
...
# def init_excel_log(path):
#     wb = openpyxl.Workbook()
#     ws = wb.active
#     ws.title = "Training Log"

#     headers = [
#         "Episode", "Timestep", "Episode Reward",
#         "Avg Reward (last 10)", "Cumulative Avg Reward",
#         "Distance Covered (m)", "Deviation from Center (m)"
#     ]
...
# def append_excel_row(path, row_data):
...
def train():
    ...
    scores = list()
    # recent_scores = deque(maxlen=10)          # <-- for Avg Reward (last 10)
    ...
    while timestep < TRAIN_TIMESTEPS:
        ...
        deviation_from_center += ep_deviation
        distance_covered += ep_distance

        scores.append(current_ep_reward)
        # recent_scores.append(current_ep_reward)
        ...
        print(
            ...
            ', Distance Covered:{}'.format(info[0])
        )

        # --- Save to Excel every episode ---
        # append_excel_row(EXCEL_LOG_PATH, [
        #     episode,
        #     timestep,
        #     round(current_ep_reward, 4),
        #     round(avg_last_10, 4),
        #     round(cumulative_score, 4),
        #     round(ep_distance, 4),
        #     round(ep_deviation, 4),
        # ])
