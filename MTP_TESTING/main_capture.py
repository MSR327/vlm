...
def run():
    ...
    while folder_count < NO_OF_TEST_EPISODES+1:
        ...
        for t in range(EPISODE_LENGTH):
            ...
            mean,action = agent(observation)
            observation, reward, done, info = env.step(action.numpy().flatten())
        ...
        print('Episode: {}'.format(episode),', Timestep: {}'.format(timestep),', Reward:  {:.2f}'.format(current_ep_reward),', Distance Covered:{}'.format(info[0]))
